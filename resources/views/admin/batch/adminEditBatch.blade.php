...
<div class="d-flex flex-column" id="content-wrapper">
    <div id="content-1">
        ...
        <div class="m-3">
            ...
            <form action="/batch/{{$batch->id}}" method="post" enctype="multipart/form-data">
                {{ csrf_field() }}
                ...
                <div class="mb-3 mt-3">
                    ...
                    <select class="form-control" name="course_id" id="course_id" required>
                        ...
                        <option value="{{$course->id}}" {{$batch->course->id == $course->id ? 'selected' : ''}}>
                            {{$course->course_name}}
                        </option>
                        @endforeach
                ...
                <div class="mb-3 mt-3">
                    ...
                    <input type="text" class="form-control" name="batch_name" id="batch_name"
                        placeholder="Please provide batch name" value="{{$batch->batch_name}}" required>
                    @error('batch_name')
                    <span class="text-danger">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="mb-3 mt-3">
                    ...
                    <select class="form-control" name="batch_shift" id="batch_shift" required>
                        ...
                        <option value="afternoon" {{$batch->shift == 'afternoon' ? 'selected' : ''}}> Afternoon
                        </option>
                        <option value="night" {{$batch->shift == 'night' ? 'selected' : ''}}> Night </option>
                    </select>
                    @error('batch_shift')
                    <span class="text-danger">{{ $message }}</span>
                    @enderror
                ...
                <div class="mb-3 mt-3">
                    ...
                    <input type="date" class="form-control w-50" name="starting_date" id="starting_date"
                        value="{{$batch->starting_date}}" required>
                    @error('starting_date')
                    <span class="text-danger">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="mb-3 mt-3">
                    ...
                    <input type="date" class="form-control w-50" name="ending_date" id="ending_date"
                        value="{{$batch->ending_date}}" required>
                    @error('ending_date')
                    <span class="text-danger">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="mb-3 mt-3">
                    ...
                    <textarea class="form-control" name="description" id="description" cols="30" rows="10"
                        require>{{$batch->description}} </textarea>
                    @error('description')
                    <span class="text-danger">{{ $message }}</span>
                    @enderror
                </div>
